...
### alertmanager & Rules for K8S

#### Rules

##### 配置告警规则rules-configmap
...
##### rules-configmap添加挂载至prometheus-deployment
...
##### 添加rules配置至prometheus-configmap
...
##### 重新加载configmap与deployment

```shell
kubectl apply -f prometheus*-configmap.yaml
kubectl apply -f pormetheus-deployment.yaml
#等待pod重启，查看规则
```

![image-20231213110340715](./images/image-20231213110340715.png)
...
```shell
cat > alertmanager-tem-configmap.yaml << EOF
#自定义告警模板
apiVersion: v1
kind: ConfigMap
metadata:
...
##### 添加alertmanager配置至prometheus-configmap

```shell
alerting:
      alertmanagers:
      - static_configs:
          - targets: ["10.0.0.36:9093"]
```
...
```shell
#查看日志
kubectl logs $(kubectl get pod | grep alert | awk '{print$1}')
#coredns无法解析smtp.qq.com
ts=2023-12-13T08:48:31.593Z caller=dispatch.go:352 level=error component=dispatcher msg="Notify for alerts failed" num_alerts=1 err="default-receiver/email[0]: notify retry canceled after 16 attempts: establish connection to server: dial tcp: lookup smtp.qq.com on 10.0.0.2:53: server misbehaving"
#手动添加解析至coredns ConfigMap
...
#进入alertmamager pod尝试解析
kubectl exec -it $(kubectl get pod | grep alert | awk '{print$1}') -- /bin/sh
nslookup smtp.qq.com
```

###### 告警发送成功

![image-20231213172152716](./images/image-20231213172152716.png)

##### 创建alertmanager-configmap（企业微信webhook）
...
kubectl delete pod $(kubectl get pod | grep alert | awk '{print$1}')
```

##### 创建webhook-adapter中转deployment

```shell
cat > webhook-adapter-deployment.yaml << EOF
---
kind: Service
apiVersion: v1
metadata:
...
kubectl apply -f webhook-adapter-deployment.yaml
```
...
curl -XPOST -d"$alerts_message" http://1.13.91.214:30093/api/v1/alerts
```

![image-20231214154020683](./images/image-20231214154020683.png)
...
![v2-6510f4ef31e693d6b4b6345439854c4a_1440w](./images/v2-6510f4ef31e693d6b4b6345439854c4a_1440w.webp)
...
节点可用内存需>2G，否则会造成节点一直pending

```shell
cat > es-quickstart.yaml << EOF
...
![image-20231216172331107](./images/image-20231216172331107.png)

```shell
kubectl get pod -l common.k8s.elastic.co/type=elasticsearch
```

![image-20231217183625154](./images/image-20231217183625154.png)

```shell
kubectl get secret -l common.k8s.elastic.co/type=elasticsearch
```

![image-20231217183755647](./images/image-20231217183755647.png)

##### 获取es密码

```shell
ELASTIC_PASSWORD=$(kubectl get secrets quickstart-es-elastic-user -o=jsonpath='{.data.elastic}' | base64 --decode)
echo $ELASTIC_PASSWORD
7s08fWTms063y1IdfHP5N13O
```

![image-20231217184113098](./images/image-20231217184113098.png)
...
![image-20231218115105136](./images/image-20231218115105136.png)
